...
export default function EmailStep({ redirectPath = '/', title, minimal = false }: { redirectPath?: string; title?: string; minimal?: boolean } = {}) {
  ...
  const sendLink = async () => {
    ...
    try { if (redirectPath !== '/') localStorage.setItem('post_login_path', redirectPath); else localStorage.removeItem('post_login_path') } catch { /* ignore */ }
    // 온보딩 답변을 서버에 잠깐 보관 — 링크를 다른 브라우저/앱에서 열어도 질문을 다시 안 해도 되게.
    // 꺼낼 때는 로그인 링크에 실린 무작위 토큰이 필요 (남이 내 이메일로 답변을 심어둘 수 없음)
    let stashToken: string | null = null
    try {
      const raw = localStorage.getItem('pending_answers')
      if (raw) {
        stashToken = crypto.randomUUID().replace(/-/g, '')
        await supabase.rpc('stash_onboarding', { p_email: email.trim(), p_answers: { ...JSON.parse(raw), refSource: localStorage.getItem('ref_source') ?? undefined }, p_research: localStorage.getItem(RESEARCH_CONSENT_KEY) === '1', p_token: stashToken })
      }
    } catch { stashToken = null /* 보관 실패해도 로그인은 진행 (같은 브라우저면 localStorage로 이어짐) */ }
    const { error } = await supabase.auth.signInWithOtp({
      email: email.trim(),
      // 메일 링크는 항상 실제 사이트로 돌아오게 고정 — 개발용 localhost에서 요청해도 꺼진 서버로 돌아가는 일 방지
      options: { emailRedirectTo: stashToken ? `https://uscollegeroadmap.com/?st=${stashToken}` : 'https://uscollegeroadmap.com' },
    })
    setSending(false)
    if (error)
      setError(
  ...
  return (
    <div>
      ...
      {!minimal && (
        <p className="mt-2 text-center text-[11px] text-gray-400">{t('계속하면 ', 'By continuing you agree to the ')}<button type="button" onClick={() => navigate('/terms')} className="underline">{t('이용약관', 'Terms')}</button>{t('과 ', ' and ')}<button type="button" onClick={() => navigate('/privacy')} className="underline">{t('개인정보처리방침', 'Privacy Policy')}</button>{t('에 동의한 것으로 봐요.', '.')}</p>
      )}
      {!emailOpen && (
        <button onClick={() => setEmailOpen(true)} className="mt-5 block w-full text-center text-xs text-gray-400 underline">
          {t('기존 이메일 계정으로 로그인', 'Log in with an existing email account')}
        </button>
      )}
      {emailOpen && (
      <div className="my-4 flex items-center gap-3 text-xs text-gray-400">
        <span className="h-px flex-1 bg-gray-200" />{t('기존 이메일 계정으로', 'existing email account')}<span className="h-px flex-1 bg-gray-200" />
      </div>
      )}
