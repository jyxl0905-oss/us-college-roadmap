...
// 미술·디자인 전문학교(kind='art') 자동 추천 — 학생 전공이 아래 창작 계열이면,
// 그 전공을 공식 개설한 전문학교만 골라 보여줌 (개설 전공은 각 학교 공식 입학처 확인분: art_programs)
...
const artSchools = (schoolsIndex as ArtSchoolLite[]).filter((s) => s.kind === 'art')
...
export const artSchoolCount = artSchools.length
