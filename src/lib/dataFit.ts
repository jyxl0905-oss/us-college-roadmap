...
//   · 합격률 10% 미만 → 무조건 Reach (초저합격률 학교는 점수와 무관하게 도전) — 국제학생은 국제학생 합격률, 미국 시민권·영주권자는 전체 합격률
...
export function dataFitOf(profile: ProfileRow, school: School): Fit | null {
  ...
  const rate = profile.applicant_status === 'domestic' ? (school.overall_accept_rate ?? null) : school.intl_accept_rate
  if (rate !== null && rate < 10) return 'reach'
