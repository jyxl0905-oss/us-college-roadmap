...
export const planStatusKo: Record<PlanStatus, string> = bilingual(
  { planned: '계획', doing: '진행 중', done: '완료' },
...
// 축 짧은 라벨 (칩용 — 글자 수 최소)
export const axisShort: Record<Axis, string> = bilingual(
...
export function cycleSeasons(now: Date = new Date()): { label: string; ko: string }[] {
  ...
  return [
    { label: `${fallYear}-fall`, ko: t('가을', 'Fall') },
    { label: `${fallYear + 1}-spring`, ko: t('봄', 'Spring') },
    { label: `${fallYear + 1}-summer`, ko: t('여름', 'Summer') },
  ]
}
