...
export default function PlansTab({ userId, majorKey }: PlansTabProps) {
  ...
  if (!plans) return <AppShell tab="plans" title="내 계획"><p className="mt-10 text-center text-gray-400">불러오는 중…</p></AppShell>

  const add = async () => {
    const t = title.trim()
    if (!t) return
    const row = await insertRow<Plan>('plans', userId, { title: t, axis, season_label: season, status: 'planned', notes: null })
  ...
  return (
    <AppShell tab="plans" title="내 계획">
            ...
            <div className="mt-2 flex flex-col gap-1.5">
              ...
              {list.map((p) => (
                <div key={p.id} className={`flex items-center gap-2 rounded-xl border-2 bg-white px-3 py-2 ${p.status === 'done' ? 'border-gray-100' : 'border-gray-200'}`}>
                  ...
                  <button onClick={() => remove(p.id)} aria-label="삭제" className="shrink-0 text-gray-300 active:text-red-500">✕</button>
                </div>
              ))}
            </div>
          </div>
      ...
      {plans.some((p) => !seasons.some((s) => s.label === p.season_label)) && (
        <div className="mt-5">
          <h2 className="text-sm font-semibold text-gray-500">지난 학년도</h2>
          <div className="mt-2 flex flex-col gap-1.5">
            {plans.filter((p) => !seasons.some((s) => s.label === p.season_label)).map((p) => (
              <div key={p.id} className="flex items-center gap-2 rounded-xl border-2 border-gray-100 bg-white px-3 py-2">
                ...
                <button onClick={() => remove(p.id)} aria-label="삭제" className="text-gray-300 active:text-red-500">✕</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {active.length === 0 && (
        <p className="mt-6 text-center text-xs text-gray-400">계획을 하나 적으면 리포트 차트에 점선이 생겨요.</p>
      )}
    </AppShell>
  )
}
