...
export default function ActivitiesTab({ userId }: ActivitiesTabProps) {
  ...
  // ─── 수상 CRUD ───
  const saveHonor = async (draft: Omit<Honor, 'id' | 'sort_order'>, id: number | 'new') => {
    if (id === 'new') {
  ...
  return (
    <AppShell tab="activities" title={t('활동 · 수상', 'Activities · Honors')}>
      ...
      <div className="mt-3 flex flex-col gap-2.5">
        {activities.map((a, i) =>
          editing === a.id ? (
            <ActivityForm key={a.id} initial={a} onSave={(d) => saveActivity(d, a.id)} onCancel={() => setEditing(null)} onDelete={() => removeActivity(a.id)} />
          ) : (
            <div key={a.id} className="rounded-xl border-2 border-gray-200 bg-white px-4 py-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex shrink-0 flex-col gap-1">
                  <button onClick={() => move(i, -1)} disabled={i === 0} className="rounded-md px-2 py-0.5 text-gray-400 disabled:opacity-20 active:bg-gray-100">▲</button>
                  <button onClick={() => move(i, 1)} disabled={i === activities.length - 1} className="rounded-md px-2 py-0.5 text-gray-400 disabled:opacity-20 active:bg-gray-100">▼</button>
                </div>
              </div>
        ...
        {editing === 'new' ? (
          <ActivityForm onSave={(d) => saveActivity(d, 'new')} onCancel={() => setEditing(null)} />
        ) : (
      ...
      <div className="mt-3 flex flex-col gap-2.5">
        ...
        {editingHonor === 'new' ? (
          <HonorForm activities={activities} onSave={(d) => saveHonor(d, 'new')} onCancel={() => setEditingHonor(null)} />
        ) : (
    ...
    <div className="rounded-xl border-2 border-blue-600 bg-white px-4 py-4">
      ...
      <div className="mt-1 flex gap-2">
        {GRADES.map((g) => {
          ...
          return (
            <button key={g} type="button" onClick={() => setD({ ...d, grades: on ? d.grades.filter((x) => x !== g) : [...d.grades, g].sort() })}
...
// ─── 수상 폼 ───
function HonorForm({
  initial, activities, onSave, onCancel, onDelete,
}: {
  initial?: Honor
  activities: Activity[]
